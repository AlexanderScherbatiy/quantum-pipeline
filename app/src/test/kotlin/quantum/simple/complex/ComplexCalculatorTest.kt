...
class ComplexCalculatorTest {
    ...
    fun testSum() {
        val calc = getCalculator()
        assertComplex(calc.calculate(ComplexOne + ComplexImage), 1.0, 1.0)
        val c1 = CartesianComplex(1.0, 2.0)
        val c2 = CartesianComplex(3.0, 4.0)
        assertComplex(calc.calculate(c1 + c2), 4.0, 6.0)
    }

    @Test
    fun testMul() {
        val calc = getCalculator()
        assertComplex(calc.calculate(ComplexOne * ComplexImage), 0.0, 1.0)
        assertComplex(calc.calculate(ComplexOne * ComplexOne), 1.0, 0.0)
        assertComplex(calc.calculate(ComplexImage * ComplexImage), -1.0, 0.0)
        val c1 = CartesianComplex(1.0, 2.0)
        val c2 = CartesianComplex(3.0, 4.0)
        assertComplex(calc.calculate(c1 * c2), -5.0, 10.0)
    }
}
